...
class ASPP(nn.Module):
    ...
    def profileModule(self, input: Tensor, **kwargs) -> Union[Tensor, float, float]:
        params, macs = 0.0, 0.0
        res = []
        for c in self.Convs:
            out, p, m = moduleProfile(module=c, x=input)
            params += p
            macs += m
            res.append(out)
        res = torch.cat(res, dim=1)

        out, p, m = moduleProfile(module=self.Project, x=res)
        params += p
        macs += m
...
class ASPPPooling(nn.Module):
    ...
    def profile_module(self, input: Tensor) -> Tuple[Tensor, float, float]:
        out, params, macs = moduleProfile(module=self.AsppPool, x=input)
        out = F.interpolate(
